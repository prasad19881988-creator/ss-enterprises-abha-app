...
## v4.3 corrections
- Owner remains the highest role; Owner account is not removable through normal staff controls.
- Added Team Leader login and team workspace with a server-enforced maximum of 10 staff per TL.
- Added secure server sessions; private APIs no longer accept unauthenticated writes.
- API state responses redact stored passwords.
- Preserved soft-delete state across restarts (archived staff are no longer silently reactivated).
- Kept 38 Bihar districts and existing customer/card/team data structure.
- GPS routine updates do not create a backup snapshot on every location update.
- Added owner-only staff archive endpoint with audit logging.
- Added direct phone-call action and browser meeting entry point for team communication.

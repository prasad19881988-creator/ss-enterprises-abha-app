...
// --- MongoDB Atlas Connection & State Model ---
...
// Save database to MongoDB Atlas + Local Backup
async function saveDatabase(reason='update') {
  try {
    if(reason && reason!=='routine') backupDatabase(reason);
    
    // Save to MongoDB Atlas Cloud
...
// Load database from MongoDB Atlas on startup
async function loadDatabase() {
  try {
    await mongoose.connect(MONGODB_URI);
    console.log('✅ Connected to MongoDB Atlas Successfully!');

    const doc = await AppState.findOne({ key: 'main' });
    if (doc && doc.data) {
      dbState = normalizeDatabase(doc.data);
      console.log('📦 Database loaded from MongoDB Atlas.');
    } else {
      dbState = normalizeDatabase(defaultDbState);
      await saveDatabase('initial-seed');
      console.log('🌱 Initial database seeded to MongoDB Atlas.');
    }
  } catch (err) { 
...
app.post('/api/signup', async (req,res) => {
  ...
  const saved = await saveDatabase('staff-register');
  if(!saved){dbState.employees.pop();g.members=g.members.filter(x=>x!==id); if(tlId){const tl=dbState.teamLeaders.find(t=>t.id===tlId); if(tl) tl.staffIds=tl.staffIds.filter(x=>x!==id);} return res.status(500).json({ok:false,message:'Registration save nahi ho saki.'});}
  res.json({ok:true,employee:e,password:'SS@12345',group:g});
});

app.post('/api/district-coordinator-signup', async (req,res)=>{
  try {
    ...
    const saved = await saveDatabase('dc-register');
    if(!saved){dbState.districtHeads.pop();return res.status(500).json({ok:false,message:'Coordinator account save nahi ho saka.'});}
...
app.post('/api/staff-control', async (req,res)=>{
  if(!requireRole(req,res,['owner','district_head','team_leader']))return;
  const id=clean(req.body.id), action=clean(req.body.action), district=clean(req.body.district);
  const e=dbState.employees.find(x=>x.id===id);
  if(district && String(e?.district||e?.area||'').toLowerCase()!==district.toLowerCase())return res.status(403).json({ok:false,message:'Staff is not in this district.'});
  if(!e)return res.json({ok:false,message:'Staff not found.'});
  if(action==='disable')e.enabled=false;
  else if(action==='enable')e.enabled=true;
  else if(action==='logout')e.status='Offline';
  else return res.status(400).json({ok:false,message:'Invalid action.'});
  const saved = await saveDatabase();
  if(!saved)return res.status(500).json({ok:false,message:'Control change save nahi hua.'});
  res.json({ok:true,employee:e});
});
...
app.post('/api/work-status', async (req,res)=>{
  ...
  const saved = await saveDatabase();
  if(!saved)return res.status(500).json({ok:false,message:'Work status save nahi hua.'}); 
  res.json({ok:true,employee:e});
});
...
app.post('/api/location-sync',async(req,res)=>{
  ...
  const saved = await saveDatabase('routine');
  if(!saved)return res.status(500).json({ok:false,message:'Location save nahi hui.'});
...
app.post('/api/card-update', async (req,res)=>{
  ...
  const saved = await saveDatabase();
  if(!saved)return res.status(500).json({ok:false,message:'Card update save nahi hua.'});
...
app.post('/api/add-customer', async (req,res)=>{
  ...
  const e=dbState.employees.find(x=>x.id===empId); if(clientId){const existing=dbState.customers.find(x=>x.clientId===clientId);if(existing)return res.json({ok:true,duplicate:true,customer:existing});} 
  const c={id:'CUS-'+Date.now().toString(36).toUpperCase(),clientId,employeeId:e?e.id:(empId||'SYSTEM'),employeeName:e?e.name:'Field Staff',name,mobile:mobile||'—',abha:abha||'—',date:getISTDate()};
  dbState.customers.unshift(c);if(e)e.cards=dbState.customers.filter(x=>x.employeeId===e.id).length;
  const saved = await saveDatabase();
  if(!saved){dbState.customers.shift();return res.status(500).json({ok:false,message:'Customer entry save nahi hui.'});}
  res.json({ok:true,customer:c});
});

app.post('/api/delete-customer', async (req,res)=>{
  if(!requireRole(req,res,['owner','district_head','team_leader','employee']))return;
  const id=clean(req.body.id),c=dbState.customers.find(x=>x.id===id);if(!c)return res.json({ok:false,message:'Customer entry not found.'});
  c.deletedAt=getISTDate();c.deletedBy=clean(req.body.actor)||'SYSTEM';c.active=false;audit('CUSTOMER_ARCHIVE',c.deletedBy,{customerId:id});
  const saved = await saveDatabase('customer-archive');
  if(!saved)return res.status(500).json({ok:false,message:'Archive save nahi hua.'});
...
app.get('/api/team-leaders',(req,res)=>{
  const district=normalizeDistrict(req.query.district||'');let list=dbState.teamLeaders.filter(t=>t.active!==false);
  if(district)list=list.filter(t=>normalizeDistrict(t.district)===district);
  res.set('Cache-Control','no-store');
  res.json({ok:true,teamLeaders:list.map(t=>{const {password,...safe}=t;return {...safe,staff:t.staffIds.map(id=>dbState.employees.find(e=>e.id===id)).filter(Boolean)}})});
});

app.post('/api/team-leader/create', async (req,res)=>{
  if(!requireRole(req,res,['owner']))return;
  const name=clean(req.body.name),phone=clean(req.body.phone),district=normalizeDistrict(req.body.district),password=typeof req.body.password==='string'?req.body.password:'';
  if(name.length<2||!/^[0-9]{10}$/.test(phone)||!district)return res.status(400).json({ok:false,message:'Name, 10 digit mobile aur district required.'});
  if(password.length<6)return res.status(400).json({ok:false,message:'Password minimum 6 characters.'});
  if(!BIHAR_DISTRICTS.includes(district))return res.status(400).json({ok:false,message:'Bihar ka valid district select kijiye.'});
  if(dbState.teamLeaders.some(t=>t.phone===phone&&t.active!==false))return res.status(409).json({ok:false,message:'Is mobile se TL already registered hai.'});
  let max=0;dbState.teamLeaders.forEach(t=>{const m=String(t.id).match(/^TL(\d+)$/);if(m)max=Math.max(max,+m[1]);});
  const id='TL'+String(max+1).padStart(3,'0');
  const tl={id,name,phone,district,password,status:'Active',active:true,maxStaff:10,staffIds:[]};
  dbState.teamLeaders.push(tl);ensureTeamGroup(tl);audit('TL_REGISTER',id,{district});
  const saved = await saveDatabase('tl-register');
  if(!saved)return res.status(500).json({ok:false,message:'TL save nahi hua.'});
  res.json({ok:true,teamLeader:{id,name,phone,district,status:'Active',maxStaff:10,staffIds:[]},password});
});

app.post('/api/team-leader/assign', async (req,res)=>{
  if(!requireRole(req,res,['owner','district_head','team_leader']))return;
  const tlId=clean(req.body.tlId),employeeId=clean(req.body.employeeId),tl=dbState.teamLeaders.find(t=>t.id===tlId&&t.active!==false),e=dbState.employees.find(x=>x.id===employeeId&&x.active!==false);
  if(!tl||!e)return res.status(404).json({ok:false,message:'TL ya staff nahi mila.'});
  if(req.user.role==='team_leader' && tl.id!==req.user.id)return res.status(403).json({ok:false,message:'TL sirf apni team manage kar sakta hai.'});
  if(!canAccessDistrict(req,e.district))return res.status(403).json({ok:false,message:'District access denied.'});
  if(normalizeDistrict(tl.district)!==normalizeDistrict(e.district))return res.status(400).json({ok:false,message:'TL aur staff same district mein hone chahiye.'});
  if(tl.staffIds.length>=10&&!tl.staffIds.includes(e.id))return res.status(400).json({ok:false,message:'TL ke under maximum 10 staff allowed hain.'});
  for(const other of dbState.teamLeaders)other.staffIds=other.staffIds.filter(id=>id!==e.id);
  tl.staffIds.push(e.id);e.teamLeaderId=tl.id; 
  const tg=ensureTeamGroup(tl); if(!tg.members.includes(e.id))tg.members.push(e.id); 
  for(const g of dbState.groups.filter(x=>x.type==='team'&&x.teamLeaderId!==tl.id))g.members=g.members.filter(id=>id!==e.id);
  audit('TL_ASSIGN',clean(req.body.actor)||'SYSTEM',{tlId,employeeId});
  const saved = await saveDatabase('tl-assign');
  if(!saved)return res.status(500).json({ok:false,message:'Assignment save nahi hua.'});
  res.json({ok:true,teamLeader:tl,employee:e});
});

app.get('/api/team-state',(req,res)=>{
  const role=clean(req.query.role),id=clean(req.query.id),district=normalizeDistrict(req.query.district);
  let employees=dbState.employees, tls=dbState.teamLeaders.filter(t=>t.active!==false);
  if(role==='district_head') {employees=employees.filter(e=>normalizeDistrict(e.district)===district);tls=tls.filter(t=>normalizeDistrict(t.district)===district);} 
  else if(role==='team_leader'){const tl=tls.find(t=>t.id===id);if(!tl)return res.status(404).json({ok:false,message:'TL not found.'});employees=employees.filter(e=>tl.staffIds.includes(e.id));tls=tl?[tl]:[];} 
  else if(role==='employee'){employees=employees.filter(e=>e.id===id);tls=tls.filter(t=>t.id===(employees[0]?.teamLeaderId||''));}
  res.set('Cache-Control','no-store');
  res.json({ok:true,district:district||null,teamLeaders:tls,employees});
});

app.get('/api/location-history',(req,res)=>{
  const employeeId=clean(req.query.employeeId);
  if(req.user.role==='employee' && req.user.id!==employeeId)return res.status(403).json({ok:false,message:'Sirf apni location history dekh sakte hain.'});
  const target=dbState.employees.find(e=>e.id===employeeId);
  if(!target)return res.status(404).json({ok:false,message:'Employee not found.'});
  if(req.user.role!=='owner' && req.user.role!=='employee' && !canAccessDistrict(req,target.district))return res.status(403).json({ok:false,message:'District access denied.'});
  const limit=Math.min(5000,Math.max(1,Number(req.query.limit||1000)));
  let rows=dbState.locationHistory;if(employeeId)rows=rows.filter(x=>x.employeeId===employeeId);
  res.set('Cache-Control','no-store');
  res.json({ok:true,history:rows.slice(-limit)});
});

app.post('/api/work-update', async (req,res)=>{
  if(!requireRole(req,res,['employee','team_leader','district_head','owner']))return;
  const employeeId=clean(req.body.employeeId),e=dbState.employees.find(x=>x.id===employeeId);
  if(!e)return res.status(404).json({ok:false,message:'Employee not found.'});
  const u={id:'WORK-'+Date.now().toString(36).toUpperCase()+'-'+Math.random().toString(36).slice(2,6),employeeId,employeeName:e.name,district:e.district||e.area||'—',type:clean(req.body.type)||'hourly',text:clean(req.body.text),cards:Number(req.body.cards||0),date:getISTDate(),dateMs:Date.now()};
  if(!u.text&&!u.cards)return res.status(400).json({ok:false,message:'Work update required.'});
  dbState.workUpdates.push(u);audit('WORK_UPDATE',employeeId,{type:u.type});
  const saved = await saveDatabase('work-update');
  if(!saved)return res.status(500).json({ok:false,message:'Work update save nahi hua.'});
  res.json({ok:true,update:u});
});

app.get('/api/work-updates',(req,res)=>{
  const employeeId=clean(req.query.employeeId),district=normalizeDistrict(req.query.district);
  let rows=dbState.workUpdates;if(employeeId)rows=rows.filter(x=>x.employeeId===employeeId);
  if(district)rows=rows.filter(x=>normalizeDistrict(x.district)===district);
  res.json({ok:true,updates:rows.slice(-1000).reverse()});
});

app.post('/api/notification', async (req,res)=>{
  if(!requireRole(req,res,['owner','district_head','team_leader']))return;
  const n={id:'NTF-'+Date.now().toString(36).toUpperCase(),to:clean(req.body.to),from:clean(req.body.from)||'SYSTEM',title:clean(req.body.title),text:clean(req.body.text),date:getISTDate(),read:false};
  if(!n.to||!n.text)return res.status(400).json({ok:false,message:'Notification recipient and text required.'});
  dbState.notifications.push(n);
  if(dbState.notifications.length>20000)dbState.notifications=dbState.notifications.slice(-20000);
  const saved = await saveDatabase('notification');
  if(!saved)return res.status(500).json({ok:false,message:'Notification save nahi hua.'});
  res.json({ok:true,notification:n});
});

app.get('/api/notifications',(req,res)=>{
  const to=clean(req.query.to);
  res.json({ok:true,notifications:dbState.notifications.filter(n=>!to||n.to===to||n.to==='ALL').slice(-200).reverse()});
});

app.post('/api/meeting', async (req,res)=>{
  if(!requireRole(req,res,['owner','district_head','team_leader','employee']))return;
  const m={id:'MTG-'+Date.now().toString(36).toUpperCase(),createdBy:clean(req.body.createdBy)||'SYSTEM',scope:clean(req.body.scope)||'ALL',district:normalizeDistrict(req.body.district),title:clean(req.body.title)||'SS ENTERPRISES Training / Meeting',roomId:clean(req.body.roomId)||crypto.randomUUID(),mode:clean(req.body.mode)||'video',screenShare:false,date:getISTDate(),dateMs:Date.now(),status:'scheduled'};
  dbState.meetings.push(m);audit('MEETING_CREATE',m.createdBy,{meetingId:m.id,scope:m.scope,district:m.district});
  const saved = await saveDatabase('meeting');
  if(!saved)return res.status(500).json({ok:false,message:'Meeting save nahi hui.'});
  res.json({ok:true,meeting:m});
});

app.get('/api/meetings',(req,res)=>{
  const district=normalizeDistrict(req.query.district);
  let rows=dbState.meetings;if(district)rows=rows.filter(m=>!m.district||normalizeDistrict(m.district)===district);
  res.json({ok:true,meetings:rows.slice(-100).reverse()});
});

app.get('/api/audit-log',(req,res)=>{
  const limit=Math.min(5000,Math.max(1,Number(req.query.limit||500)));
  res.json({ok:true,log:dbState.auditLog.slice(-limit).reverse()});
});

app.get('/api/backup-status',(req,res)=>{
  let backups=[];
  try{const dir=path.join(DATA_DIR,'backups');if(fs.existsSync(dir))backups=fs.readdirSync(dir).filter(x=>x.endsWith('.json')).sort().reverse().slice(0,20).map(name=>({name,size:fs.statSync(path.join(dir,name)).size}));}catch{}
  res.json({ok:true,schemaVersion:dbState.schemaVersion,backups,records:{employees:dbState.employees.length,customers:dbState.customers.length,cardUpdates:dbState.cardUpdates.length,locations:dbState.locationHistory.length,workUpdates:dbState.workUpdates.length,audit:dbState.auditLog.length}});
});

app.get('/api/groups',(req,res)=>{
  let groups=dbState.groups;
  const employeeId=clean(req.query.employeeId),district=normalizeDistrict(req.query.district||'');
  if(employeeId){const e=dbState.employees.find(x=>x.id===employeeId);if(e&&e.teamLeaderId){groups=groups.filter(g=>g.type==='team'&&g.teamLeaderId===e.teamLeaderId);}else if(e){groups=groups.filter(g=>g.type!=='team'&&normalizeDistrict(g.district)===normalizeDistrict(e.district));}}
  else if(district){groups=groups.filter(g=>normalizeDistrict(g.district)===district);}
  res.set('Cache-Control','no-store');
  res.json({ok:true,groups:groups.map(g=>({...g,members:g.members.map(id=>{const e=dbState.employees.find(x=>x.id===id);if(e)return e;const tl=dbState.teamLeaders.find(t=>t.id===id);return tl?{id:tl.id,name:tl.name,role:'Team Leader'}:null}).filter(Boolean)}))});
});

app.get('/api/groups/:groupId/messages',(req,res)=>{
  const g=clean(req.params.groupId);
  res.json({ok:true,messages:dbState.messages.filter(m=>m.groupId===g).slice(0,100).reverse()});
});
...
app.post('/api/group-message', async (req,res)=>{
  ...
  if(empId.startsWith('HEAD:')) {
    ...
    const saved = await saveDatabase('message');
    if(!saved)return res.status(500).json({ok:false,message:'Message save nahi hua.'}); 
...
app.post('/api/staff-archive', async (req,res)=>{
  if(!requireRole(req,res,['owner']))return;
  ...
  const saved = await saveDatabase('staff-archive');
  if(!saved)return res.status(500).json({ok:false,message:'Staff archive save nahi hua.'});
...
app.post('/api/create-group', async (req,res)=>{
  ...
  if(!district)return res.json({ok:false,message:'District required.'});
  const g=ensureGroup(district);
  const saved = await saveDatabase('group');
  if(!saved)return res.status(500).json({ok:false,message:'Group save nahi hua.'});
...
// Load database from MongoDB first, then start server
loadDatabase().then(() => {
  server.listen(PORT,'0.0.0.0',()=>console.log(`🚀 SS Enterprises gateway running on port ${PORT}`));
});

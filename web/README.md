...
## Production upgrade 4.0

This build preserves the existing JSON records and adds a versioned migration layer. Runtime data is stored in `DATA_DIR` (use `/var/data` on Render with a persistent disk).

Added foundations:
- All 38 Bihar districts.
- Team Leader hierarchy with a hard server-side limit of 10 staff per TL.
- Team-group records for TL + assigned staff.
- Append-only location history and work-update history.
- Audit log and automatic rolling JSON backups before important writes.
- Customer archive/soft-delete instead of permanent deletion.
- Notification, meeting and media metadata collections.
- Refresh/logout controls.
- Hourly work-update endpoint.
- Official Aadhaar Face RD package launch target (`in.gov.uidai.facerd`).

For real production calling/video/screen sharing, configure a WebRTC/SFU media service and TURN server; browser UI alone cannot provide reliable large-group calling on every mobile network.


## Production Fix 4.0.1
- Fixed server startup/migration order so the 38-district migration cannot run before district configuration is initialized.
- Added `/health` endpoint for Render health checks and included `render.yaml`.
- Location history is no longer truncated at 100,000 entries; existing history is preserved.
- GPS sync no longer creates a full backup snapshot on every location update, reducing disk usage and slowdowns.
- Added a premium app icon and polished startup splash screen so the installed PWA opens with a clean SS ENTERPRISES branded screen instead of a plain/green loading view.
- Existing staff, customers, card updates, groups and previous v4 features remain in the project.


### Persistent production data
Render is configured with a persistent disk mounted at `/var/data` and `DATA_DIR=/var/data`. Customer, staff, TL/DC, card, location-history, work-update, meeting, message, audit and backup data are therefore stored outside the deploy filesystem. Existing data in the persistent disk is loaded instead of being replaced by the seed file.

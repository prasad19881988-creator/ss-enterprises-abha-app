...
function normalizeDatabase(data) {
  ...
  safe.employees.forEach(e => { if (typeof e.enabled !== 'boolean') e.enabled = true; if (!Array.isArray(e.locationHistoryIds)) e.locationHistoryIds=[]; if (!e.active) e.active=true; });
...
app.get('/health', (req,res) => res.json({ ok:true, employees:dbState.employees.length, customers:dbState.customers.length, version:'4.0.0',schemaVersion:dbState.schemaVersion }));
...
app.post('/api/location-sync',async(req,res)=>{
  ...
  dbState.locationHistory.push(loc); e.locationHistoryIds.push(loc.id); if(dbState.locationHistory.length>100000) dbState.locationHistory=dbState.locationHistory.slice(-100000); audit('LOCATION_SYNC',e.id,{lat,lng,accuracy:e.accuracy});
  ...
  if(!saveDatabase('location'))return res.status(500).json({ok:false,message:'Location save nahi hui.'});
...
app.get('/api/location-history',(req,res)=>{const employeeId=clean(req.query.employeeId),limit=Math.min(5000,Math.max(1,Number(req.query.limit||1000)));let rows=dbState.locationHistory;if(employeeId)rows=rows.filter(x=>x.employeeId===employeeId);res.set('Cache-Control','no-store');res.json({ok:true,history:rows.slice(-limit)});});
app.post('/api/work-update',(req,res)=>{const employeeId=clean(req.body.employeeId),e=dbState.employees.find(x=>x.id===employeeId);if(!e)return res.status(404).json({ok:false,message:'Employee not found.'});const u={id:'WORK-'+Date.now().toString(36).toUpperCase()+'-'+Math.random().toString(36).slice(2,6),employeeId,employeeName:e.name,district:e.district||e.area||'—',type:clean(req.body.type)||'hourly',text:clean(req.body.text),cards:Number(req.body.cards||0),date:getISTDate(),dateMs:Date.now()};if(!u.text&&!u.cards)return res.status(400).json({ok:false,message:'Work update required.'});dbState.workUpdates.push(u);if(dbState.workUpdates.length>50000)dbState.workUpdates=dbState.workUpdates.slice(-50000);audit('WORK_UPDATE',employeeId,{type:u.type});if(!saveDatabase('work-update'))return res.status(500).json({ok:false,message:'Work update save nahi hua.'});res.json({ok:true,update:u});});
...
app.post('/api/group-message',(req,res)=>{
  ...
  if(empId.startsWith('HEAD:')) {
    ...
    dbState.messages.push(m); if(dbState.messages.length>50000)dbState.messages=dbState.messages.slice(-50000);
  ...
  dbState.messages.push(m);if(dbState.messages.length>50000)dbState.messages=dbState.messages.slice(-50000);if(!saveDatabase('message'))return res.status(500).json({ok:false,message:'Message save nahi hua.'});res.json({ok:true,message:m});
});

app.post('/api/create-group',(req,res)=>{const district=clean(req.body.district);if(!district)return res.json({ok:false,message:'District required.'});const g=ensureGroup(district);if(!saveDatabase('group'))return res.status(500).json({ok:false,message:'Group save nahi hua.'});res.json({ok:true,group:g});});
